...
| `orderByLevel` | 排序（**旧**）：true = 按等级需求升序，false/缺省 = 按名字。**只是排序，不是过滤条件**。新前端改用 `sortBy`，此字段只为不破坏旧调用方而保留 | ✅ |
| `sortBy` | 排序（**新**，2026-09-13）：`created`（入库时间，新→旧，前端默认）/ `quality`（品质→等级→名称）/ `level`（**等级降序**→品质→名称）/ `name`（名称→品质→等级）。非空时**优先于** `orderByLevel`。SQL 见 `PlayerItemDaoImpl.BuildOrderBy` | ✅ |
