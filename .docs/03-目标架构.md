...
| `rarity` / `prefixRarity` | 品质。⚠️ 值是数据库里的 `Yellow`/`Green`/`Blue`/`Epic`，**不是** `Legendary` | ✅ |
...
```json
{
  ...
  "slots":     [{ "value": "ArmorProtective_Head", "labelTag": "iatag_slot_head", "label": "头盔" }],
