...
| `GET` | `/api/collection` | `RequestCollectionData()` |
...
> **实现状态（2026-09-11）**：以上端点均已在 `tools/devapi` 落地，
> 其中 `/api/items/transfer` 只在**沙盒写模式**下可用。
> 现状、差异与安全机制见 [`04-开发环境.md`](./04-开发环境.md) §8。
...
| ⚠️ **C# HTTP 服务用什么** | `HttpListener`（.NET 自带，简单）vs Kestrel（现代，但要引入 ASP.NET Core）。倾向 `HttpListener` |
| ⚠️ **端口策略** | 固定 vs 动态 |
| ⚠️ **是否需要认证 token** | 本地服务是否要防本机其他程序 |
